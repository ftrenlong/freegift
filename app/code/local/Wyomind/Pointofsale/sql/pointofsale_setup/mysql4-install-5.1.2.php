...
if (version_compare(Mage::getVersion(), '1.5.0', '>=')) {
    ...
    if (Mage::getSingleton('core/resource')->getConnection('core_write')->isTableExists($table)) {
        $installer->run(
            "INSERT INTO `{$this->getTable('pointofsale')}` (`place_id`,`store_code`,`name`,`main_phone`,`address_line_1`,`address_line_2`,`city`,`state`,`postal_code`,`country_code`,`hours`,`description`,`longitude`,`latitude`,`status`,`image`,`customer_group`,`position`,`store_id`)
