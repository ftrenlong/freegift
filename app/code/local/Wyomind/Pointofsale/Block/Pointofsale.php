...
class Wyomind_Pointofsale_Block_Pointofsale extends Mage_Core_Block_Template
{
    ...
    public function getPointofsale($checkout = false) 
    {
        $filter = $checkout ? "visible_checkout" : "visible_google_map";
        if (Mage::app()->getStore()->getStoreId())
            $collection = Mage::getModel('pointofsale/pointofsale')->getPlacesByStoreId(Mage::app()->getStore()->getStoreId())->addFieldToFilter($filter, Array($filter => 1))->setOrder('`position`', 'ASC');
        else
    ...
    public function getCountries($checkout = false) 
    {
        $filter = $checkout ? "visible_checkout" : "visible_google_map";
        $collection = Mage::getModel('pointofsale/pointofsale')->getCountries(Mage::app()->getStore()->getStoreId())->addFieldToFilter($filter, Array($filter => 1));
        $countries = array();
    ...
    public function renderJs($checkout = false) 
    {
        $html = " 
                 W_GP = {
                strings:{
...
";
        $i = 0;
        $data = array();
        foreach ($this->getPointofsale($checkout) as $place) {
            ...
            $data[] = array(
                ...
                "links" => array(
                    ...
                    "showOnMap" => "<a target='_blank' href='//maps.google.com/maps?q=" . $request . "'>" . Mage::helper('pointofsale')->__("Show on Google Map") . "</a>"
                ),
                "name" => $place->getName(),
    ...
    public function renderScroller($checkout = false) 
    {
        $html = null;
        foreach ($this->getPointofsale($checkout) as $place) {
