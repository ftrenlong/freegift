...
class Wyomind_Advancedinventory_Model_Api extends Mage_Api_Model_Resource_Abstract
{
    ...
    Public function insertLogRow($context, $action, $reference, $details,
            $user = false) 
    {
        try {
            if (!$user)
    ...
    public function fetchOrderByWhPos($warehouse_id, $from = null, $to = null) 
    {
        ...
        try {
            ...
            if ($from) {
                $orderCollection->addAttributeToFilter("created_at", array("gteq" => $from));
            }
            if ($to) {
                $orderCollection->addAttributeToFilter("created_at", array("lt" => $to));
            }

            $orderCollection->getSelect()->where($warehouse_id . " IN (assignation_warehouse)");
            
            foreach ($orderCollection as $order) {
                $assigned = array();
                foreach (Mage::helper("core/data")->jsonDecode($order->getAssignationStock()) as $product_id => $dispatch) {
                    foreach ($dispatch as $wh_id => $qty) {
                        if ($wh_id == $warehouse_id) {
                            $assigned[$product_id] = $qty;
                        }
                    }
                }
                $data[] = array(
