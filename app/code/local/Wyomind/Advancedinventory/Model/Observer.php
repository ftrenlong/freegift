...
class Wyomind_Advancedinventory_Model_Observer
{
        ...
        foreach ($orders as $order) {
            ...
            if (!$order->getAssignationStock()) {
                ...
                if (Mage::getStoreConfig("advancedinventory/setting/autoassign_order")) {
                    ...
                    foreach (array_unique($result->assignation_warehouses) as $warehouse_id) {

                        // Get the destination email addresses to send copies to
                        $emails = array_merge($emails, explode(',', trim(Mage::getModel('pointofsale/pointofsale')->load($warehouse_id)->getInventoryNotification())));
                    }
                    ...
                    if ($order->getState() != Mage_Sales_Model_Order::STATE_CANCELED) {
                        ...
                        if (Mage::getStoreConfig("advancedinventory/setting/admin_order_notification") != "") {
    ...
    function getAvailability($product, $type = null)
    {
        ...
        if (!$modelInventory->getIsInStock() || $product->getStatus() == 2) {
            return false;
        }
    ...
    public function addActionColumn(Varien_Event_Observer $observer)
    {
        ...
        if ($block instanceof Mage_Adminhtml_Block_Catalog_Product_Grid && Mage::getStoreConfig("advancedinventory/system/enableproductgrid")) {
    ...
    function productSave($observer)
    {
        ...
        if (!in_array($product->getTypeId(), array("configurable", "bundle", "grouped", "virtual"))) {
            ...
            if ($store_id || !$is_admin) {
                ...
                if (isset($temp_inventory[$product_id]) && isset($temp_inventory[$product_id]['pos_wh']))
            ...
            if ($multistock_enabled) {
                ...
                foreach ($inventory[$product_id]['pos_wh'] as $pos_id => $invData) {
                    ...
                    if (($stock->getId() == 0 /* && $qty */) || ($stock->getId() > 0 && ($stock->getQuantity_in_stock() != $qty || $stock->getUse_config_setting_for_backorders() != $use_config_setting_for_backorders || $stock->getManageStock() != $manage_stock || $stock->getBackorder_allowed() != $backorder_allowed))) {
            ...
            $product->addData(array('stock_data' => array_merge($product->getData('stock_data'), $updatedData['stock_data'])));
        }
        return $observer;
    }
