...
// Vérification stricte
        ...
        foreach ($items as $item) {
            if ($item->getQtyOrdered() > 0 && in_array($item->getProductType(), array('simple', 'virtual', 'downloadable', 'grouped'))) {
                ...
                if ($item->getParentItemId() != null && $items[$i - 1]->getProductType() == "configurable") {
                    $qty = $item->getQtyOrdered() - $items[$i - 1]->getOrigData('qty_refunded') - $items[$i - 1]->getQtyCanceled();
                    if (isset($orderedItems[$item->getProductId()]['qty'])) {
                        $orderedItems[$item->getProductId()]['qty'] += $qty;
                    } else {
                        $orderedItems[$item->getProductId()]['qty'] = $qty;
                    }
                } else {
                ...
                $orderedItems[$item->getProductId()]['product_id'] = $item->getProductId();
    ...
    public function getAssignation($order, $orderedItems)
    {
        ...
        $this->decreaseStock($orderedItems, $this->assignation_stock, array("action" => "Order placed", "type" => $type, "store_id" => $order->getStoreId(), "order_id" => $order->getId(), "warehouses" => $this->assignation_warehouses, "user" => null));
       
        return $this;
    }

    public function checkProductAvailability($item, $place, $mutiple_assignation = true,
            $pickup = false, $places = array())
    {
        ...
        if ($productId != '') {
            ...
            $assigned_qty = 0;
            if (!$pickup)
                if (isset($this->assignation_stock[$item['id']]) && $this->assignation_stock[$item['id']] != null)
    ...
    public function decreaseStock($orderedItems, $assignation = array(),
            $additional = array("type" => "", "action" => "", "store_id" => "", "user" => ""),
            $sign = 1)
    {
