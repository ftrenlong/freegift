...
class Wyomind_Advancedinventory_Block_Stocks extends Mage_Core_Block_Template
{
    ...
    function _toHtml() 
    {
        ...
        foreach ($places as $p) {
            ...
            if ($p->getVisibleProductPage() != 1 || !in_array(Mage::app()->getStore()->getStoreId(), explode(',', $p->getStoreId())) || !in_array($customer_id, explode(',', $p->getCustomerGroup())) || !$managelocalstock)
                continue;
            $c++;
