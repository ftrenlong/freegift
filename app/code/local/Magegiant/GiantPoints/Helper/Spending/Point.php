 ...
 */
class Magegiant_GiantPoints_Helper_Spending_Point extends Magegiant_GiantPoints_Helper_Calculation_Abstract
{
    /**
    ...
    {
        if (!Mage::helper('giantpoints/config')->isEnabled(Mage::getModel('giantpoints/customer')->getStoreId())) {
            return false;
        }
        ...
        if (!Mage::getModel('giantpoints/customer')->isAllowSpend($this->getQuote()->getStoreId())) {
            return false;
        }
    ...
    public function getSpendingRules()
    {
        ...
        Mage::helper('giantpoints/cache')->saveCache($cacheKey, $container->getSpendingRules());
    ...
    {
        return Mage::getModel('giantpoints/customer')->getBalance();
    }
    ...
    {
        if (!Mage::helper('giantpoints/cache')->hasCache('customer_point')) {
            $points = $this->getCustomerTotalPoints();
            $points -= $this->getCalculation()->getPointItemSpent();
            $points -= $this->getCalculation()->getCheckedRuleSpentPoint();
            if ($points < 0) {
                $points = 0;
            }
            $maxPoints = Mage::helper('giantpoints/calculation_spending')->getMaxPointsPerOrder($quote->getStoreId());
            if ($maxPoints && $maxPoints < $points) {
                $points = $maxPoints;
            }
            Mage::helper('giantpoints/cache')->saveCache('customer_point', $points);
        }

        return Mage::helper('giantpoints/cache')->getCache('customer_point');
    }
    ...
    {
        return Mage::getModel('giantpoints/customer')->getCustomer();
    }
    ...
    {
        $store = Mage::app()->getStore(Mage::getModel('giantpoints/customer')->getStoreId());
        if ($rule->getId() == 'rate') {
    ...
    {
        if (!Mage::helper('giantpoints/cache')->hasCache('checked_data')) {
            $session            = Mage::getSingleton('checkout/session');
            $rewardCheckedRules = $session->getRewardCheckedRules();
            if (!is_array($rewardCheckedRules)) {
                Mage::helper('giantpoints/cache')->saveCache('checked_data', array());
            } else {
                Mage::helper('giantpoints/cache')->saveCache('checked_data', array_keys($rewardCheckedRules));
            }
        }

        return Mage::helper('giantpoints/cache')->getCache('checked_data');
    }
