...
class Magegiant_GiantPoints_Model_Total_Invoice_Point extends Mage_Sales_Model_Order_Invoice_Total_Abstract
{
    ...
    protected function _addSpentPointDiscount($invoice)
    {
        ...
        $maxDiscount       = $order->getGiantpointsDiscount();
        $maxBaseDiscount   = $order->getGiantpointsBaseDiscount();
        $totalDiscount     = 0;
        $baseTotalDiscount = 0;
        ...
        foreach ($invoice->getAllItems() as $item) {
            ...
            if ($orderItemDiscount && $orderItemQty) {
                ...
                $maxItemDiscount     = $maxDiscount - $totalDiscount;
                $maxItemBaseDiscount = $maxBaseDiscount - $baseTotalDiscount;
                if ($discount > $maxItemDiscount) {
                    $discount     = $maxItemDiscount;
                    $baseDiscount = $maxItemBaseDiscount;
                }
