...
class Magegiant_GiantPoints_Model_Mysql4_Transaction_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract
{
    ...
    public function filterByStatusCondition($condition)
    {
        $transactions = $this->addFieldToFilter('status', $condition);
    ...
    public function sendEmailBeforeExpire($beforeDays, $storeIds = array())
    {
        $transactions = $this->addBalanceActiveFilter()->addExpiredAfterDaysFilter($beforeDays);
        if (count($storeIds)) {
        ...
        foreach ($transactions as $transaction) {
            try {
                ...
                $transaction->setData('reward_customer', $rewardAccount->load($transaction->getRewardId()));
                $transaction->setData('customer', $customer->load($transaction->getCustomerId()));
                if ($transaction->sendEmailBeforeExpire())
        ...
        if (count($transactionIds)) {
            try {
                $this->getResource()->increaseExpireEmail($transactionIds);
            } catch (Exception $e) {
                Mage::helper('giantpoints')->log('Exception: ' . $e->getMessage() . ' in ' . __CLASS__ . ' on line ' . __LINE__);
            }
        }
