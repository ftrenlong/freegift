...
class Magegiant_GiantPoints_Model_Block_Observer
{
    ...
    public function appendPaymentSpending($block, $transport)
    {
        if ($block instanceof Mage_Checkout_Block_Onepage_Payment_Methods) {
            ...
            $html      = $transport->getHtml();
            $isOnePage = false;
            if (Mage::helper('core')->isModuleEnabled('Magecheckout_SecuredCheckout') && Mage::helper('securedcheckout/config')->isEnabled()) {
                $append_html = $block->getLayout()
                    ...
                    ->renderView();
            } else if (Mage::helper('core')->isModuleEnabled('Magecheckout_SecuredCheckoutPro') && Mage::helper('securedcheckout/config')->isEnabled()) {
                $append_html = $block->getLayout()
                    ->createBlock('giantpoints/checkout_onepage_payment_spending')
                    ->setTemplate('magegiant/giantpoints/checkout/onestepcheckout/payment/magecheckout_spending.phtml')
                    ->renderView();
