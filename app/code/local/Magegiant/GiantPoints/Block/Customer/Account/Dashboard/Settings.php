...
class Magegiant_GiantPoints_Block_Customer_Account_Dashboard_Settings extends Magegiant_GiantPoints_Block_Abstract
{
    ...
    /**
     * get current reward account
     *
     * @return mixed
     */
    public function getRewardAccount()
    {
        $giantPointsAccount = $this->getRewardCustomer();
        if (!$giantPointsAccount->getId()) {
            $giantPointsAccount->setNotificationUpdate(0)
                ->setNotificationExpire(0);
        }

        return $giantPointsAccount;
    }
