 ...
 */
class Magegiant_GiantPoints_Adminhtml_Giantpoints_TransactionController extends Mage_Adminhtml_Controller_Action
{
    /**
    ...
    public function massCompleteAction()
    {
        ...
        if (!is_array($tranIds) || !count($tranIds)) {
            ...
            $transactions = Mage::getResourceModel('giantpoints/transaction_collection')->filterByStatusCondition(array(
                'lt' => Magegiant_GiantPoints_Model_Transaction::STATUS_COMPLETED
            ));
            $transactions->addFieldToFilter('point_amount', array('gt' => 0));
            $transactions->addFieldToFilter('transaction_id', array('in' => $tranIds));
            $total = 0;
    ...
    public function massExpireAction()
    {
        ...
        if (!is_array($tranIds) || !count($tranIds)) {
            ...
            $transactions->addAvailableBalanceFilter();
            $transactions->addFieldToFilter('expiration_date', array('notnull' => true));
            $transactions->addFieldToFilter('expiration_date', array('lteq' => $curDateTime));
            $transactions->addFieldToFilter('transaction_id', array('in' => $tranIds));
            $total = 0;
            foreach ($transactions as $transaction) {
                try {
